...
class MainWindow(QMainWindow):
    ...
    def _build_dashboard(self):
        ...
        # 1. Ribbon Organization (Pillar 6)
        ribbon_frame = QFrame()
        ribbon_frame.setObjectName("Ribbon")
        ribbon_layout = QHBoxLayout(ribbon_frame)
        ribbon_layout.setContentsMargins(10, 10, 10, 10)

        # 1. 🚀 Quick Actions
        btn_actions = QPushButton("🚀 Quick Actions ▾")
        btn_actions.setProperty("class", "LaunchBtn")
        ...
        # Hooks for Phase 2: Headless Messenger
        ...
        btn_data.setProperty("class", "PrimaryAction")
        ...
        menu_data.addAction("🔗 Connect Existing Profile Folders").triggered.connect(self.profile_service.connect_existing_profiles) # New Power Feature
        btn_data.setMenu(menu_data)

        # 3. 🍪 Session Management Menu
        btn_session = QPushButton("🍪 Session Management ▾")
        btn_session.setProperty("class", "SecondaryAction")
        ...
        # Note: Export Session is usually single-profile so left out of bulk or handled specifically
        btn_session.setMenu(menu_session)

        # 4. ⚙️ Tools Menu
        btn_tools = QPushButton("⚙️ Tools ▾")
        btn_tools.setProperty("class", "DangerAction")
        ...
        self.table.setHorizontalHeaderLabels(["[x]", "Profile Name", "Status", "Group", "Proxy", "Email", "⚙️ Manage"])
    ...
    def _build_settings(self):
        ...
        btn_sync.clicked.connect(lambda: QMessageBox.information(self, "OAuth Integration", "Google OAuth Flow placeholder ready for API integration."))
    ...
    def load_table_data(self):
        ...
        for p in profiles:
            ...
            act_launch = menu.addAction("🚀 Open Browser")
            act_launch.triggered.connect(lambda checked, prof=p: self._launch_single(prof))

            menu.addAction("🔄 Update Proxy").triggered.connect(lambda checked, pid=p['id']: self.profile_service.manage_single_proxy(pid))
            menu.addAction("📤 Export This Session").triggered.connect(lambda checked, prof=p: self.profile_service.export_single_cookie(prof))
            menu.addAction("🗑️ Delete This Profile").triggered.connect(lambda checked, pid=p['id']: self.profile_service.execute_bulk_delete([pid]))
    ...
    def _launch_single(self, profile):
        worker = BrowserTaskWorker(profile, self.app_dir)
    ...
    def _dispatch_messenger_pulse(self, mode: str):
        ...
        QMessageBox.information(self, "Messenger Pulse Started", f"Headless Messenger dispatched in {mode} mode.\nDiscord Webhook: {'Active' if url_text else 'Inactive'}")
